...
class SST5(Dataset):
    def __init__(self, examples, tokenizer, text_transform=None):
        ...
        self.texts = [" ".join(t.split()[:max_seq_length]) if len(t.split()) > max_seq_length else t for t in
                      self.texts]  # clip texts
        self.labels = [label_map[d.label] for d in examples]
        self.aug_n_dist = 0
